		...
		>
			<div className="space-y-4">
				...
				<form className="space-y-4" onSubmit={handleSubmit}>
					<div className="flex flex-col gap-1">
						...
						<input
							...
							placeholder={t(
								"dashboard.sections.installments.placeholders.name"
							)}
					...
					<div className="flex flex-col gap-2">
						...
						<div className="flex flex-wrap gap-2">
							...
							>
								{t(
									"dashboard.sections.installments.modes.total"
								)}
							</button>
							<button
								type="button"
							...
							>
								{t(
									"dashboard.sections.installments.modes.monthly"
								)}
							</button>
						</div>
					</div>
					...
					<div className="grid gap-3 sm:grid-cols-2">
						<div className="flex flex-col gap-1">
							...
							>
								{mode === "total"
									? t(
											"dashboard.sections.installments.labels.totalAmount"
									  )
									: t(
											"dashboard.sections.installments.labels.monthlyAmount"
									  )}
							</label>
							<input
								...
								placeholder={
									mode === "total"
										? t(
												"dashboard.sections.installments.placeholders.totalAmount"
										  )
										: t(
												"dashboard.sections.installments.placeholders.monthlyAmount"
										  )
								}
								type="number"
								min="0"
						...
						<div className="flex flex-col gap-1">
							...
							>
								{t(
									"dashboard.sections.installments.labels.count"
								)}
							</label>
							<input
								...
								placeholder={t(
									"dashboard.sections.installments.placeholders.count"
								)}
								type="number"
								min="1"
								step="1"
					...
					<div className="flex flex-col gap-1">
						...
						<input
							...
							placeholder={t(
								"dashboard.sections.installments.placeholders.note"
							)}
					...
					{preview ? (
						<div className="rounded-2xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-3 text-xs text-emerald-700 shadow-inner dark:border-emerald-400/30 dark:bg-emerald-400/10 dark:text-emerald-200">
							<p>
								{t(
									"dashboard.sections.installments.preview.helper"
								)}
							</p>
							<div className="mt-2 flex flex-wrap gap-x-6 gap-y-1">
								{typeof preview.total === "number" ? (
									<span>
										{t(
											"dashboard.sections.installments.preview.total",
											{
								...
								{typeof preview.monthly === "number" ? (
									<span>
										{t(
											"dashboard.sections.installments.preview.monthly",
											{
												amount: formatCurrency(
													preview.monthly
												),
				...
				<div
					className={
						items.length > 3
							? "max-h-56 overflow-y-auto pr-2 scrollable-section"
							: ""
					}
				>
							...
							items.map((item) => {
								...
								if (
									...
									const current =
										...
										1;
									scheduleInfo = ` ${t(
										"dashboard.sections.installments.currentOverTotal",
										{
											current,
									...
									scheduleInfo = ` ${t(
										"dashboard.sections.installments.remaining",
										{
									...
									scheduleInfo = ` ${t(
										"dashboard.sections.installments.planned",
										{
									...
									>
										<div className="space-y-1">
											...
											<p className="text-xs text-neutral-500 dark:text-neutral-400">
												{t(
													"dashboard.sections.installments.perMonth",
													{
														amount: formatCurrency(
															monthly,
															{
											...
											{item.totalAmount ? (
												<p className="text-xs text-neutral-400 dark:text-neutral-500">
													{t(
														"dashboard.sections.installments.total",
														{
															amount: formatCurrency(
																item.totalAmount,
																{
										...
										>
											{t(
												"dashboard.sections.installments.remove"
											)}
										</button>
									</li>
								);
