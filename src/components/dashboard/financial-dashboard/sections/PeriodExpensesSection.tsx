...
type PeriodExpensesSectionProps = {
  items: MoneyItem[];
  summaryLabel: string;
  countLabel: string;
  formState: {
...
export const PeriodExpensesSection = ({
  items,
  summaryLabel,
  countLabel,
  formState,
  ...
  return (
    <SectionCard
      title={t("dashboard.sections.periodExpenses.title")}
      subtitle={t("dashboard.sections.periodExpenses.subtitle")}
    >
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-2 text-xs font-medium text-amber-700 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-100">
          <span>{summaryLabel}</span>
          <span className="rounded-lg bg-amber-500/20 px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-amber-800 dark:bg-amber-400/30 dark:text-amber-50">
            {countLabel}
          </span>
        </div>
        <form className="space-y-3" onSubmit={handleSubmit}>
          <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_150px_auto]">
            <div className="flex flex-col gap-1">
              ...
              >
                {t("dashboard.sections.periodExpenses.labels.name")}
              </label>
              <input
                id="period-name"
                ...
                placeholder={t("dashboard.sections.periodExpenses.placeholders.name")}
                value={formState.name}
                onChange={handleNameChange}
                aria-invalid={Boolean(errors.name)}
                aria-describedby={errors.name ? "period-name-error" : undefined}
              />
            ...
            <div className="flex flex-col gap-1">
              <label
                ...
                className="text-xs font-semibold uppercase tracking-wide text-neutral-400 dark:text-neutral-500"
              >
                {t("dashboard.sections.periodExpenses.labels.amount")}
              </label>
              <input
                id="period-amount"
                className={`rounded-xl border px-3 py-2 sm:py-3 text-sm h-10 sm:h-12 shadow-sm transition focus:outline-none focus:ring-2 ${
                ...
                placeholder={t("dashboard.sections.periodExpenses.placeholders.amount")}
                type="number"
                min="0"
                step="0.01"
                ...
                aria-describedby={errors.amount ? "period-amount-error" : undefined}
              />
              <div className="h-6 mt-1">
                {errors.amount ? (
                  <p
                    id="period-amount-error"
                    className="text-sm font-medium text-rose-500 dark:text-rose-400"
                    role="alert"
                  >
                    {errors.amount}
            ...
            <div className="flex flex-col justify-end">
              <div className="h-10 sm:h-12 flex items-center">
                <button
                  ...
                  disabled={!isValid}
                >
                  {t("dashboard.sections.periodExpenses.add")}
                </button>
              </div>
          ...
          <div className="flex flex-col gap-1">
            <label
              ...
              className="text-xs font-semibold uppercase tracking-wide text-neutral-400 dark:text-neutral-500"
            >
              {t("dashboard.sections.periodExpenses.labels.note")}
            </label>
            <input
              id="period-note"
              ...
              placeholder={t("dashboard.sections.periodExpenses.placeholders.note")}
          ...
          <ul className="space-y-2">
            {items.length === 0 ? (
              <li className="rounded-xl border border-dashed border-neutral-200/80 bg-white/50 px-4 py-3 text-sm text-neutral-500 dark:border-neutral-700 dark:bg-neutral-900/60 dark:text-neutral-400">
                {t("dashboard.sections.periodExpenses.empty")}
              </li>
            ) : (
              items.map((item) => (
                <li
                ...
                >
                  ...
                  >
                    {t("dashboard.sections.periodExpenses.remove")}
                  </button>
                </li>
              ))
            )}
