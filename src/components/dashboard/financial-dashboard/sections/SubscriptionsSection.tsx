...
export const SubscriptionsSection = ({
  ...
  return (
    <SectionCard
      title={t("dashboard.sections.subscriptions.title")}
      subtitle={t("dashboard.sections.subscriptions.subtitle")}
    >
        ...
        <form className="space-y-3" onSubmit={handleSubmit}>
          <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_150px_auto]">
            <div className="flex flex-col gap-1">
              ...
              >
                {t("dashboard.sections.subscriptions.labels.name")}
              </label>
              <input
                ...
                placeholder={t("dashboard.sections.subscriptions.placeholders.name")}
            ...
            <div className="flex flex-col gap-1">
              <label
                ...
                className="text-xs font-semibold uppercase tracking-wide text-neutral-400 dark:text-neutral-500"
              >
                {t("dashboard.sections.subscriptions.labels.amount")}
              </label>
              <input
                ...
                placeholder={t("dashboard.sections.subscriptions.placeholders.amount")}
                type="number"
                min="0"
                step="0.01"
            ...
            <div className="flex flex-col justify-end">
              <div className="h-10 sm:h-12 flex items-center">
                <button
                  ...
                  disabled={!isValid}
                >
                  {t("dashboard.sections.subscriptions.add")}
                </button>
              </div>
          ...
          <div className="flex flex-col gap-1">
            <label
              ...
              className="text-xs font-semibold uppercase tracking-wide text-neutral-400 dark:text-neutral-500"
            >
              {t("dashboard.sections.subscriptions.labels.note")}
            </label>
            <input
              ...
              placeholder={t("dashboard.sections.subscriptions.placeholders.note")}
          ...
          <ul className="space-y-2">
            {items.length === 0 ? (
              <li className="rounded-xl border border-dashed border-neutral-200/80 bg-white/50 px-4 py-3 text-sm text-neutral-500 dark:border-neutral-700 dark:bg-neutral-900/60 dark:text-neutral-400">
                {t("dashboard.sections.subscriptions.empty")}
              </li>
            ) : (
              items.map((item) => (
                <li
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-400">
                      {t("dashboard.sections.subscriptions.perMonth", {
                        amount: formatCurrency(item.amount, { locale: intlLocale, currency }),
                      })}
                    </p>
                    {item.notes ? (
                  ...
                  >
                    {t("dashboard.sections.subscriptions.remove")}
                  </button>
                </li>
              ))
            )}
